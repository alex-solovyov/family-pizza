...
export const Item = React.memo(({ item, countInCart }) => {
    ...
    const onAddToCart = () => {
        ...
        dispatch(actions.addPizzaToCart(itemForCart));
    }
    ...
    return (
        <div className="pizza-block">
            ...
            <div className="pizza-block__selector">
                <ul>
                    {types.map((type, index) => <li key={index} onClick={() => setActiveType(index)} className={classNames({
                        active: activeType === index,
                        disabled: !item.types.includes(index)
                    })}>{type}</li>)}
                </ul>
                <ul>
                    {sizes.map(size => <li key={size} onClick={() => setActiveSize(size)} className={classNames({
            ...
            <div className="pizza-block__bottom">
                ...
                <div className="button button--outline button--add" onClick={onAddToCart}>
                    <svg
                        width="12"
                        height="12"
                        viewBox="0 0 12 12"
